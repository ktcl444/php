...
class Solution extends \algorithm\base\AlgorithmBase
{
    ...
    function leastInterval1($tasks, $n) {
        $char_array = array_fill(0,26,0);
		$length = count($tasks);
		for($i = 0 ;$i < $length ;$i++)
		{
			$char_array[ord($tasks[$i])-ord('A')]++;
		}
		sort($char_array);
		$time = 0;
		while($char_array[25]>0){
				$i = 0;
            while ($i <= $n) {
                if ($char_array[25] == 0)
                    break;
                if ($i < 26 && $char_array[25 - $i] > 0)
                    $char_array[25 - $i]--;
                $time++;
                $i++;
            }
...
#region 优先队列
    function leastInterval2($tasks, $n) {
		...
		sort($char_array);
		$queue = [];
		foreach($char_array as $num){
			if($num > 0)
							array_unshift($queue,$num);
		}
		$time = 0;
        while (!empty($queue)) {
             $i = 0;
...
/* 		sort($char_array);
		$max =   $char_array[25] -1;
		$free_space = $max * $n;
		for($i = 24;$i>0 && $char_array[$i]>0;$i--)
		{
			$free_space -= min($max,$char_array[$i]);
		}
		
		return $free_space >0 ? count($tasks) + $free_space : count($tasks); */
